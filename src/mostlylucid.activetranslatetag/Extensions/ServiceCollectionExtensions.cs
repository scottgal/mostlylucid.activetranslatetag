...
public static class ServiceCollectionExtensions
{
    ...
    {
        var options = new mostlylucid.activetranslatetag.Configuration.LlmTranslateOptions();
        ...
        services.AddScoped<IAiTranslationProvider>(sp =>
        {
            ...
            var ai = options.Ai ?? new mostlylucid.activetranslatetag.Configuration.AiOptions();
            var defaultName = ai.DefaultProvider;

            // Try resolve default from Ollama list first
            mostlylucid.activetranslatetag.Services.IAiTranslationProvider? selected = null;
            ...
            if (ai.OllamaProviders != null && ai.OllamaProviders.Length > 0)
            {
                ...
                if (pick != null)
                {
                    ...
                    var provLogger = loggerFactory.CreateLogger<mostlylucid.activetranslatetag.Services.Providers.OllamaTranslationProvider>();
                    selected = new mostlylucid.activetranslatetag.Services.Providers.OllamaTranslationProvider(http, provLogger, pick.BaseUrl, pick.Model);
                }
            }
            ...
            if (selected == null && ai.EasyNmtProviders != null && ai.EasyNmtProviders.Length > 0)
            {
                ...
                if (pick != null)
                {
                    ...
                    var provLogger = loggerFactory.CreateLogger<mostlylucid.activetranslatetag.Services.Providers.EasyNmtTranslationProvider>();
                    selected = new mostlylucid.activetranslatetag.Services.Providers.EasyNmtTranslationProvider(http, provLogger, pick.BaseUrl);
                }
            }
            ...
            if (selected == null && ai.OpenAiProviders != null && ai.OpenAiProviders.Length > 0)
            {
                ...
                if (pick != null)
                {
                    ...
                    var provLogger = loggerFactory.CreateLogger<mostlylucid.activetranslatetag.Services.Providers.OpenAiTranslationProvider>();
                    selected = new mostlylucid.activetranslatetag.Services.Providers.OpenAiTranslationProvider(http, provLogger, pick.ApiKey ?? string.Empty, pick.Model);
                }
            }
            ...
            var chunk = ai.Chunking ?? new mostlylucid.activetranslatetag.Configuration.ChunkingOptions();
            if (chunk.Enabled)
            {
                var chunkLogger = loggerFactory.CreateLogger<mostlylucid.activetranslatetag.Services.Providers.ChunkingAiTranslationProvider>();
                selected = new mostlylucid.activetranslatetag.Services.Providers.ChunkingAiTranslationProvider(selected, chunk.ChunkLength, chunk.Overlap, chunkLogger);
            }
