...
public class OllamaTranslationProvider : IAiTranslationProvider
{
    ...
    public async Task<string> TranslateAsync(string text, string targetLanguage, string? sourceLanguage = "en", string? description = null, CancellationToken ct = default)
    {
        try
        {
            var prompt = BuildPrompt(text, targetLanguage, sourceLanguage, description);
    ...
    public async Task<Dictionary<string, string>> TranslateBatchAsync(Dictionary<string, string> items, string targetLanguage, string? sourceLanguage = "en", string? description = null, CancellationToken ct = default)
    {
        ...
        {
            result[key] = await TranslateAsync(value, targetLanguage, sourceLanguage, description, ct);
        }
        return result;
    }

    private static string BuildPrompt(string text, string targetLanguage, string? sourceLanguage, string? description)
    {
        var src = string.IsNullOrWhiteSpace(sourceLanguage) ? "auto" : sourceLanguage;
        var prompt = $"Translate the following text from {src} to {targetLanguage}. Preserve placeholders like {{0}} or {{name}}, and preserve HTML tags as-is.";
