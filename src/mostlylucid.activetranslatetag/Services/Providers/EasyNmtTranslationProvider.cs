...
public class EasyNmtTranslationProvider : IAiTranslationProvider
{
    ...
    public async Task<string> TranslateAsync(string text, string targetLanguage, string? sourceLanguage = "en", string? description = null, CancellationToken ct = default)
    {
        try
        {
            ...
            var payload = new
            {
                ...
                source_lang = string.IsNullOrWhiteSpace(sourceLanguage) ? "" : sourceLanguage
            };
    ...
    public async Task<Dictionary<string, string>> TranslateBatchAsync(Dictionary<string, string> items, string targetLanguage, string? sourceLanguage = "en", string? description = null, CancellationToken ct = default)
    {
        ...
        {
            var t = await TranslateAsync(v, targetLanguage, sourceLanguage, description, ct);
            result[k] = t;
        }
        return result;
    }
    ...
    private async Task<string> ReadSingleTranslationAsync(HttpResponseMessage response, string fallback, CancellationToken ct)
    {
        try
        {
            ...
            // Try array { "translated": [ "..." ] } (mostlylucid-nmt format)
            if (doc?.RootElement.TryGetProperty("translated", out var translatedProp) == true)
            {
                if (translatedProp.ValueKind == JsonValueKind.Array && translatedProp.GetArrayLength() > 0)
                {
                    var translation = translatedProp[0].GetString();
                ...
                // Try as string (older EasyNMT format)
                else if (translatedProp.ValueKind == JsonValueKind.String)
                {
    ...
    private static string[]? TryReadStringArray(JsonDocument? doc)
    {
        ...
        // Try { "translated": [...] } (mostlylucid-nmt format)
        if (doc.RootElement.TryGetProperty("translated", out var translatedArr) && translatedArr.ValueKind == JsonValueKind.Array)
        {
            var list = new List<string?>();
            foreach (var el in translatedArr.EnumerateArray())
            {
