...
public class ChunkingAiTranslationProvider : IAiTranslationProvider
{
    ...
    public async Task<string> TranslateAsync(string text, string targetLanguage, string? sourceLanguage = "en", string? description = null, CancellationToken ct = default)
    {
        ...
        {
            return await _inner.TranslateAsync(text, targetLanguage, sourceLanguage, description, ct);
        }
        ...
        {
            var part = await _inner.TranslateAsync(chunk, targetLanguage, sourceLanguage, description, ct);
    ...
    public async Task<Dictionary<string, string>> TranslateBatchAsync(Dictionary<string, string> items, string targetLanguage, string? sourceLanguage = "en", string? description = null, CancellationToken ct = default)
    {
        ...
        if (small.Count > 0)
        {
            var batchResult = await _inner.TranslateBatchAsync(small, targetLanguage, sourceLanguage, description, ct);
            foreach (var kv in batchResult) result[kv.Key] = kv.Value;
        }

        foreach (var kv in large)
        {
            result[kv.Key] = await TranslateAsync(kv.Value, targetLanguage, sourceLanguage, description, ct);
        }
